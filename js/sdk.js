...
const SDK = {
    resetData: function(callback){
      ...
      sessionStorage.setItem("data", JSON.stringify(defaultData))
        callback(null, "");
    },
    ...
    Bruger: {
        opretBruger: function(fornavn, efternavn, email, password, callback){
            if(fornavn && efternavn && email && password){
                ...
                data.brugere.push({idStudent: id, firstName: fornavn, lastName: efternavn, email: email, password: password, deltager: []});
                callback(null, "")
            } else{
    ...
    Event: {
        ...
        hentTilmeldteEvents: function (callback){
            SDK.loadData(function (err, data) {
                if(err){
                    ...
                    var user = JSON.parse(sessionStorage.getItem('bruger'));
                    function isIdStudent(bruger) {
                        return bruger.idStudent == user.idStudent;
                    }
                    var brugerIndex = data.brugere.findIndex(isIdStudent);
                    callback(null, data.brugere[brugerIndex].deltager)
                }
        ...
        deltag: function (idEvent, callback) {
            var valgteEvent = null;
            var user = JSON.parse(sessionStorage.getItem('bruger'));


            function isIdEvent(event){
                return event.idEvent == idEvent
            }
            var eventIndex = data.events.findIndex(isIdEvent)

            function isIdStudent(bruger) {
                return bruger.idStudent == user.idStudent;
            }
            var brugerIndex = data.brugere.findIndex(isIdStudent);
            data.brugere[brugerIndex].deltager.push(data.events[eventIndex])


            data.events[eventIndex].deltagere.push(user)
